$.get("https://eth-mainnet.alchemyapi.io/v2/EPx6hNqYZwrb0hhmr9nsmavrbM8b6wch/getNFTsForCollection?contractAddress=0xeB3fC95B74C79C2c3469188A72df2c2399D752AB&startToken=0&withMetadata=true",function(result){
    ...
    $.get(nfts[18].tokenUri.gateway, function(res18) {
        chapter18url = res18.external_url;
        $.get(chapter18url.toString(),function(chap18){
            $("#eighteen").html(chap18.toString());
            console.log("the guardians have ensured you are seeing chapter 18 content from " + chapter18url);
        });
    })
    ...
    $.get(nfts[20].tokenUri.gateway, function(res20) {
        chapter20url = res20.external_url;
        $.get(chapter20url.toString(),function(chap20){
            $("#twenty").html(chap20.toString());
            console.log("the guardians have ensured you are seeing chapter 20 content from " + chapter20url);
        });
    })
});
